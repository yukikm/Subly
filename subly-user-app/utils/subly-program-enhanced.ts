...
/**
 * Enhanced utility functions for interacting with the Subly Solana program using Anchor
 */
...
export function findSubscriptionServicePDA(providerWallet: PublicKey, serviceId: number): [PublicKey, number] {
  return PublicKey.findProgramAddressSync(
    [SEEDS.SUBSCRIPTION_SERVICE, providerWallet.toBuffer(), new BN(serviceId).toArrayLike(Buffer, 'le', 8)],
    SUBLY_PROGRAM_ID,
  )
}
...
export function findUserSubscriptionPDA(userWallet: PublicKey, subscriptionId: number): [PublicKey, number] {
  return PublicKey.findProgramAddressSync(
    [SEEDS.USER_SUBSCRIPTION, userWallet.toBuffer(), new BN(subscriptionId).toArrayLike(Buffer, 'le', 8)],
    SUBLY_PROGRAM_ID,
  )
}

/**
 * Create instruction for depositing SOL using manual instruction building
 * (since we don't have the full Anchor IDL setup in the client)
 */
export function createDepositSolInstruction(userWallet: PublicKey, amount: number): TransactionInstruction {
  ...
  const amountBuffer = amountLamports.toArrayLike(Buffer, 'le', 8)
...
/**
 * Create instruction for subscribing to a service using manual instruction building
 */
export function createSubscribeToServiceInstruction(
  ...
  const serviceIdBuffer = new BN(serviceId).toArrayLike(Buffer, 'le', 8)
